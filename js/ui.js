// ── 공통 UI 컴포넌트 ───────────────────────────────────────────────────────────
const Card = ({ children, className="" }) => <div className={`bg-white rounded-3xl shadow-sm ${className}`}>{children}</div>;
...
const Btn = ({ children, onClick, variant="default", size="md", className="", disabled=false }) => {
  ...
  return <button type="button" onClick={onClick} disabled={disabled} className={`inline-flex items-center justify-center font-medium rounded-2xl transition-all focus:outline-none disabled:opacity-40 cursor-pointer ${s[size]||s.md} ${v[variant]||v.default} ${className}`}>{children}</button>;
};
...
const SummaryCard = ({ title, value, description, icon }) => (
  <Card className="p-5">
    <div className="flex items-start justify-between gap-3">
      <div><div className="text-xs text-slate-500 font-medium">{title}</div><div className="mt-1 text-3xl font-bold tracking-tight">{value}</div></div>
